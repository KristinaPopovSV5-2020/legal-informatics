...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="lrml:LegalRuleML">
        <RuleML>
            ...
            <xsl:attribute name="rdf_export_classes">jail_3_months confiscation confiscate_weapon to_pay_min to_pay_max
        recommend_fine_reduction to_pay_less recommend_increased_penalty
        is_high_category_weapon_in_public to_increase_penalty 30_imprisonment money to_pay_max2 to_pay_min2</xsl:attribute>
                ...
                <slot>
                    <Ind>
                        <xsl:attribute name="uri"
                            select="concat('lc:',lower-case(../ruleml:Rel/@iri))" />
                    </Ind>
                    <Data>
                        <xsl:value-of select="." />
                    </Data>
    ...
    <xsl:template name="superiority">
        ...
        <xsl:for-each
            select="//lrml:Override[@over=concat('#',$currentPrescriptiveStatementKey)]">
            <xsl:variable name="targetPrescriptiveStatementKey" select="replace(@under,'^(#)','')" />
            <xsl:variable
                name="targetRuleKey"
                select="//lrml:PrescriptiveStatement[@key=$targetPrescriptiveStatementKey]/ruleml:Rule/@key" />
            <superior>
                <Ind>
                ...
                <body>
                    <Atom>
                        <op>
                            <Rel>
                                <xsl:value-of
                                    select="//lrml:PrescriptiveStatement[@key=$targetPrescriptiveStatementKey]//ruleml:then//ruleml:Rel" />
                            </Rel>
                        </op>
                        <slot>
                            <Ind uri="defendant" />
                ...
                <head>
                    <Atom>
                        <op>
                            <Rel>
                                <xsl:value-of
                                    select="replace(//lrml:PenaltyStatement[@key=$penaltyKey]//ruleml:Rel/@iri,'^(:)','')" />
                            </Rel>
                        </op>
                        <slot>
                            <Ind uri="value" />
                            <Data xsi:type="xs:integer">
                                <xsl:value-of
                                    select="//lrml:PenaltyStatement[@key=$penaltyKey]//ruleml:Ind" />
                            </Data>
                        </slot>
                    </Atom>
                </head>
